...
(defvar mzn/unix-agenda-path "~/Documents/Notes/Agenda")
(defvar mzn/unix-code-path "~/Documents/Code")
(defvar mzn/unix-font-size 200)

(defvar mzn/windows-dotfiles-path "C:\\SPBData\\Dotfiles")
(defvar mzn/windows-notes-path "C:\\SPBData\\Notes")
(defvar mzn/windows-agenda-path "C:\\SPBData\\Notes\\Agenda")
(defvar mzn/windows-code-path "C:\\SPBData\\Code")
(defvar mzn/windows-font-size 150)
...
;; 确保变量被正确定义
(defvar mzn/default nil)

;; 设定路径和其他相关设置（可根据需要扩展）
(if mzn/is-windows
    (progn
      (setq mzn/default 'windows)
      ;; 在这里添加 Windows 相关的配置
      )
  (progn
    (setq mzn/default 'unix)
    ;; 在这里添加 Unix 相关的配置
    ))
 #+end_src
